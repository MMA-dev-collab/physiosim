...
const PasswordInputDemo = () => {
    ...
    return (
        <div className='w-96 mx-auto p-4 border rounded-xl shadow-sm bg-white'>
            <form className='space-y-2' onSubmit={(e) => e.preventDefault()}>
                <label htmlFor='password-demo' className='block text-sm font-medium'>
                    Password Demo
                </label>
                <div className='relative'>
                    <input
                        ...
                        aria-describedby='password-strength-demo'
            ...
            <div className='flex gap-2 w-full justify-between mt-2'>
                <span
                    className={`${calculateStrength.score >= 1 ? 'bg-green-200' : 'bg-gray-200'
                        } p-1 rounded-full w-full`}
                ></span>
                <span
                    className={`${calculateStrength.score >= 2 ? 'bg-green-300' : 'bg-gray-200'
                        } p-1 rounded-full w-full`}
                ></span>
                <span
                    className={`${calculateStrength.score >= 3 ? 'bg-green-400' : 'bg-gray-200'
                        } p-1 rounded-full w-full`}
                ></span>
                <span
                    className={`${calculateStrength.score >= 4 ? 'bg-green-500' : 'bg-gray-200'
                        } p-1 rounded-full w-full`}
                ></span>
                <span
                    className={`${calculateStrength.score >= 5 ? 'bg-green-600' : 'bg-gray-200'
                        } p-1 rounded-full w-full`}
                ></span>
            </div>
